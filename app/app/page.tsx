...
// Interfaces for stub components
interface ParticleBurstProps {
  active: boolean;
}

interface OnboardingStep {
  ref: React.RefObject<HTMLDivElement | null>; // Fixed ref type
  title: string;
  description: string;
}
...
function ParticleBurst({ active }: ParticleBurstProps): React.ReactNode { return null }
function OnboardingSpotlight({ steps, onFinish }: OnboardingSpotlightProps): React.ReactNode { return null }

// Command interface for AI
interface AICommand {
  amount: string;
  to: string;
}
...
export default function EnhancedMoneyTransferPage(): React.ReactNode {
  // ─── State & Refs ────────────────────────────────────────────
    ...
    <motion.div className="relative min-h-screen bg-gradient-to-br from-indigo-50 via-white to-pink-50 p-6 overflow-hidden">
      ...
      <div ref={headerRef} className="sticky top-0 bg-white bg-opacity-70 backdrop-blur-md p-4 rounded-b-2xl shadow-md max-w-xl mx-auto mb-6 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          ...
          <select 
            value={language} 
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setLanguage(e.target.value as 'en'|'zu')} 
      ...
      <div ref={balanceRef} className="relative bg-white p-5 rounded-2xl shadow-md max-w-xl mx-auto mb-6">
        ...
        <div className="mt-4 flex items-center space-x-2">
          ...
          <select 
            id="currency" 
            value={currency} 
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCurrency(e.target.value as keyof typeof rates)} 
